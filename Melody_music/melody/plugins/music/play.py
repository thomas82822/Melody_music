...
async def _play_core(client: Client, message: Message, video: bool = False, force: bool = False):
    ...
    try:
        ...
        try:
            ...
            # Only delete the "Getting your vibe ready..." placeholder AFTER
            # reply_photo succeeds — if the chat lacks photo permission
            # (ChatSendPhotosForbidden), reply_photo raises and we fall to
            # the except block, which reuses `processing` as a text fallback.
            # Deleting it here before the photo send would cause MESSAGE_ID_INVALID
            # in that fallback path.
            if sent and processing:
                try:
            ...
            await anim.stop()
            # BUG FIX: processing may have been deleted already (if the photo
            # send succeeded but a later step failed) — guard the edit so a
            # ChatSendPhotosForbidden fallback doesn't itself crash with
            # MESSAGE_ID_INVALID.
            try:
