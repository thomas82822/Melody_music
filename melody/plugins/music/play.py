...
async def _play_core(client: Client, message: Message, video: bool = False, force: bool = False):
    ...
    try:
        ...
        download_task = asyncio.create_task(download_audio(info["id"]))
        ...
        # Make sure the pre-join AND the pre-download have finished before
        # play_stream() decides whether the chat is "already active" — both
        # are usually done well before this line.
        await asyncio.gather(join_task, download_task)
