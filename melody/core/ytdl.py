...
async def get_playlist_entries(url_or_query: str, limit: int = 50) -> list[dict]:
    ...
    try:
        ...
        await send_error_log("get_playlist_entries failed", exc, context={"video_id": playlist_url})
        return []
...
async def get_video_info(url_or_query: str) -> dict | None:
    ...
    await send_error_log("get_video_info failed (all sources exhausted)", Exception("No results from any source"), context={"video_id": query})
    return None
    ...
    if cached:
        ...
        try:
            ...
            await send_error_log(
                ...
                exc,
                context={"chat_id": message.chat.id if message.chat else None, "user_id": message.from_user.id if message.from_user else None},
            )
            return None
