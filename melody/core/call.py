        ...
        try:
            ...
            try:
                await send_error_log(
                    ...
                    context={
                        "chat_id": chat_id,
                        "song_title": get_current(chat_id).title if get_current(chat_id) else None,
                    },
                )
            except Exception:
                pass
...
async def _prefetch_upcoming(chat_id: int) -> None:
    ...
    if queue:
        ...
        try:
            ...
            await send_error_log(
                f"prefetch of queued track failed in {chat_id}",
                exc,
                context={
                    "chat_id": chat_id,
                    "song_title": upcoming.title,
...
async def _stream_track(chat_id: int, track, video: bool = False, _retry: bool = False):
    ...
    try:
        ...
        # 📤 Send the downloaded song file to the dump chat in parallel
        # (non-blocking, best-effort) so the user gets playback instantly
        # while the file upload happens in the background.
        asyncio.create_task(_send_song_to_dump_chat(filepath, track))
