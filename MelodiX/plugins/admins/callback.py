...
from MelodiX import YouTube, app
from MelodiX.core.call import MelodiX
from MelodiX.misc import SUDOERS, db
from MelodiX.utils.database import (is_active_chat,
...
from MelodiX.utils.formatters import seconds_to_min
from MelodiX.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from MelodiX.utils.stream.autoclear import auto_clean
from MelodiX.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await MelodiX.pause_stream(chat_id)
        ...
        await MelodiX.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention)
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await MelodiX.stop_stream(chat_id)
        ...
        await MelodiX.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await MelodiX.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await MelodiX.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await MelodiX.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
